...
async function findGameClassifyApi (page:number,size:number,succeed:(data:findGameClassifyItemI[])=>void, failed:(err:any)=>void) {
    try {
        const res  =  await axios.get(`${apiRoot}/gameclassify?page=${page}&size=${size}`);
